...
async function openNewPlayerModal() {
  ...
  // Vincular uma partida é opcional: o jogador existe por conta própria
  // (JogadorModel.partidasModel é o lado inverso do @OneToMany, sem
  // nullable=false — não há exigência real no backend). O select serve
  // só como atalho pra já linkar uma partida salva, se você quiser.
  const select = document.getElementById('new-player-partida');
  select.innerHTML = `<option value="">Nenhuma / sem partida vinculada</option>`;
...
async function handleNewPlayerSubmit(dados) {
  try {
    await api.post('/jogador/criarjogador', {
      ...
      // Vínculo com partida é opcional. partidasModel é List<PartidasModel>
      // no backend — só mandamos o item se uma partida foi selecionada;
      // caso contrário vai lista vazia (jogador sem partida vinculada).
      partidasModel: dados.partidaId ? [{ id: Number(dados.partidaId) }] : [],
    });
...
function renderPlayerLinkedGame(j) {
  ...
  const partidas = j.partidasModel || [];
  ...
  container.innerHTML = partidas.map(partida => `
    <div class="game-card" style="max-width: 320px; margin-bottom: 0.75rem;" onclick='openGameViewer(${JSON.stringify(partida).replace(/'/g, "&#39;")})'>
