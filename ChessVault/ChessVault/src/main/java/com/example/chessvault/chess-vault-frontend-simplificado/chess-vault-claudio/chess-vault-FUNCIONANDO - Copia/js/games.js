  ...
  try {
    ...
    container.innerHTML = `<div class="games-grid">${partidas.map(p => `
      <div class="game-card" onclick='openGameViewer(${JSON.stringify(p).replace(/'/g, "&#39;")})'>
        <button class="game-card-delete" title="Deletar" onclick="event.stopPropagation(); handleDeleteGame('${p.nome.replace(/'/g, "\\'")}')">🗑</button>
...
async function handleDeleteGame(nome) {
  const confirmado = confirm(`Deletar a partida "${nome}"? Essa ação não pode ser desfeita.`);
  if (!confirmado) return;

  try {
    await api.delete(`${CONFIG.ENDPOINTS.PARTIDAS.DELETAR}?nome=${encodeURIComponent(nome)}`);
