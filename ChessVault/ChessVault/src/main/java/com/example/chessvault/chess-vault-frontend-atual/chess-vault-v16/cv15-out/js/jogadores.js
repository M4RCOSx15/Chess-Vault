...
async function openPlayerDetail(index) {
  ...
  const avatarHTML = j.imagemJogador
    ? `<img src="${j.imagemJogador}" alt="${j.nome}"
           ...
           onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">`
    : '';
  const fallbackHTML = `<span style="font-size:42px;">${iconeDoJogador(j.nome)}</span>`;
  ...
  document.getElementById('player-detail-content').innerHTML = `
    ...
    <div class="player-detail-hero">
      <div class="player-detail-hero-avatar">
        ${avatarHTML}
        <span style="${j.imagemJogador ? 'display:none;' : ''}font-size:42px;display:flex;align-items:center;justify-content:center;width:100%;height:100%;">${iconeDoJogador(j.nome)}</span>
      </div>
