  ...
  area.innerHTML = `
    <div class="puzzle-layout">
      ...
      <div class="puzzle-board-col">
        <div class="puzzle-board-outer">
          <div class="puzzle-ranks-col" id="puzzle-board-ranks"></div>
...
function _renderFenBoard(boardEl, ranksEl, filesEl, fen) {
  ...
  // Montar grade 8×8
  boardEl.className = 'puzzle-vboard-grid';
  ...
  if (filesEl) {
    filesEl.className = 'puzzle-files-labels';
  ...
  if (ranksEl) {
    ranksEl.className = 'puzzle-ranks-labels';
    for (let i = 8; i >= 1; i--) {
      const d = document.createElement('div');
      d.textContent = i;
...
function _criarCasa(row, col, peca, mapa) {
  ...
  sq.className = clara ? 'puzzle-sq puzzle-sq-l' : 'puzzle-sq puzzle-sq-d';
  if (peca && mapa[peca]) {
